...
class CDNLinkerTest extends PHPUnit_Framework_TestCase
{
	...
	protected function setUp() {
		$this->ctx = new blitznote\wp\cdn\URI_changer(
			'http://test.local',
			blitznote\wp\cdn\target_url_strategy_for('http://cdn.test.local'),
	...
	public function testNoModificationIfUrlsMatch() {
		$this->ctx->get_target_url = blitznote\wp\cdn\target_url_strategy_for('http://test.local');
	...
	public function testMultipleCDNs() {
		$this->ctx->rootrelative = true;
		$this->ctx->get_target_url = blitznote\wp\cdn\target_url_strategy_for('http://cdn%4%.test.local');
	...
	protected function comparePair($blog_url, $cdn_url, $root_relative, $fn_before, $fn_after) {
		...
		$this->ctx->get_target_url = blitznote\wp\cdn\target_url_strategy_for($cdn_url);
	...
	public function testExcludes2() {
		...
		$this->ctx->get_target_url = blitznote\wp\cdn\target_url_strategy_for('http://cdn.howdyags.com');
	...
	public function testExcludes3() {
		...
		$this->ctx->get_target_url = blitznote\wp\cdn\target_url_strategy_for('http://cdn.wangma.me');
	...
	public function testIncludeDirs() {
		...
		$this->ctx->get_target_url = blitznote\wp\cdn\target_url_strategy_for('http://cdn.screenrant.com');
	...
	public function testMissingWww() {
		...
		$this->ctx->get_target_url = blitznote\wp\cdn\target_url_strategy_for('http://cdn.prettyshinysparkly.com');
